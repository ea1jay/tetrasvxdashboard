    ...
    <div class="card-header">System Info</div>
      <div class="table-responsive">
        <table id="sysinfo" class="table table-condensed">
          <tbody>
            <tr>
              <?php if ($cputemp !== NULL) { ?><th>CPU Temperature</th><?php } ?>
              <?php if ($cpufreq !== NULL) { ?><th>CPU Frequency</th><?php } ?>
              <th>System Load</th>
              <th>CPU Usage</th>
              <th>Uptime</th>
            </tr>
            <tr class="gatewayinfo">
              <?php if ($cputemp !== NULL) { ?><td><?php echo $cputemp; ?> &deg;C</td><?php } ?>
              <?php if ($cpufreq !== NULL) { ?><td><?php echo $cpufreq; ?> MHz</td><?php } ?>
              <td><?php echo $sysload; ?> %</td>
              <td><?php	if (defined("SHOWPROGRESSBARS")) { ?><div class="progress"><div class="progress-bar 
                <?php if ($cpuusage < 30) 
			...
			echo "progress-bar-danger";
                ?>" role="progressbar" aria-valuenow="<?php echo $cpuusage; ?>" aria-valuemin="0" aria-valuemax="100" style="width: <?php echo $cpuusage; ?>%; color: black;"><?php echo $cpuusage; ?>%</div></div>
              <?php
                } else {
		echo $cpuusage." %";
                }
